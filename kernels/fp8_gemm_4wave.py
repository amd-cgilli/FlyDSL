...
Mfma128Accum_t = Vec.make_type(4, fx.Float32)
...
def _compute_global_swizzle(lane_id, wave_id, k_dim, max_steps):
    assert max_steps <= 4

    offsets = []
    for round in range_constexpr(max_steps):
        row = lane_id // 8 + wave_id * 8 + round * 32
        col = (lane_id % 8) * 16
        a, b = _swizzle_128(row, col)
        offsets.append(a * k_dim + b)
    return offsets

def _compute_lds_swizzle(lane_id, wave_idx, max_steps):
    assert max_steps <= 4

    lds_swz = []
    for row_offset in range_constexpr(max_steps):
        # Each wave loads a 16x128 tile of A/B, max_steps is the max number of tiles
        # we want to load.
        row = wave_idx * (max_steps * 16) + row_offset * 16 + lane_id % 16
        swz = []
...
def _load_lds(gl_src, lds_dst, k_offset, gl_offsets, wave_id, max_steps):
    assert wave_id < 2
    assert max_steps <= 4
    assert len(gl_offsets) == max_steps

    from flydsl._mlir.dialects import memref as memref_dialect
    lds_base_i = memref_dialect.extract_aligned_pointer_as_index(lds_dst)
    for step in range_constexpr(max_steps):
...
def _load_rt(lds_src, lane_id, wave_idx, n_rows):
    # Load n_rows 16x128 fragments from LDS to registers
    # Each 16x128 fragment requires 2 i32x4 (2 ds_read_b128)
    frag = []
    for i in range_constexpr(n_rows):
        row = wave_idx * (n_rows * 16) + i * 16 + lane_id % 16
        halves = []
...
def _store_C_scaled(c_frag, c_dst, c_stride, a_scale_src, b_scale_src, base_row, base_col, lane_id, n_rows, n_cols):
    for ti in range_constexpr(n_rows):
        row = base_row + ti * 16 + (lane_id // 16) * 4
        a_scale_v = Vec(buffer_ops.buffer_load(a_scale_src, fx.Int32(row), vec_width=4, dtype=fx.Float32))
        for tj in range_constexpr(n_cols):
            col = base_col + tj * 16 + lane_id % 16
            b_scale = buffer_ops.buffer_load(b_scale_src, fx.Int32(col), vec_width=1, dtype=fx.Float32)
            vec_f32 = Vec(c_frag[_c_idx(ti, tj, n_rows)])
            for i in range_constexpr(4): # this is a constant and is equal to the number of floats each thread holds for a 16x16 out tile (depends on mfma atom used)
                ...
                buffer_ops.buffer_store(scaled, c_dst, fx.Int32((row + i) * c_stride + col))
...
def _c_idx(i, j, n_rows):
    return i * n_rows + j

def _mfma_ABt_all(a, b, c, n_rows, n_cols):
    assert len(a) == n_rows
    assert len(b) == n_cols
    assert len(c) == n_rows * n_cols

    for i in range_constexpr(n_rows):
        for j in range_constexpr(n_cols):
            c[_c_idx(i, j, n_rows)] = rocdl.mfma_scale_f32_16x16x128_f8f6f4(Mfma128Accum_t, [a[i], b[j], c[_c_idx(i, j, n_rows)], 0, 0, 0, 0x7F7F7F7F, 0, 0x7F7F7F7F])
    return c
...
def compile_fp8_gemm_256x256x128(
    ...
    def kernel_gemm(
        ...
        def _mfma_ABt(a, b, c, m, n):
            c[_c_idx(m, n)] = rocdl.mfma_scale_f32_16x16x128_f8f6f4(Mfma128Accum_t, [a[m], b[n], c[_c_idx(m, n)], 0, 0, 0, 0x7F7F7F7F, 0, 0x7F7F7F7F])
            return c
...
def compile_fp8_gemm_128x128x128(
    ...
    N_ROWS = BLOCK_M // 4 // 16 # this is actually the number of 16-row tiles in a BLOCK_M x BLOCK_N tile
    N_COLS = BLOCK_N // 4 // 16
    N_ACCUMS = N_ROWS * N_COLS # Each accumulator is 4 floats (depends on MFMA atom)
    assert N_ACCUMS > 0
...
def check_gemm(size: int):
    ...
    launch_fn = compile_fp8_gemm_128x128x128(M=M, N=N, K=K)
    def _as_i8(t):
