...
def compile_mxfp8_gemm_8w(
    *,
    ...
    MAX_LDS_ROUNDS = max(N_LDS_STEPS_A, N_LDS_STEPS_B)
    ...
    def kernel_gemm(
        ...
        def _compute_global_swizzle():
            offsets = []
            # 8 waves each load 8 rows, so one round covers n_waves*8 = 64 rows;
            # the round stride must be n_waves*8, not the 4-wave value of 32.
            for round in range_constexpr(MAX_LDS_ROUNDS):
                row = lane_id // 8 + wave_id * 8 + round * 64
                col = (lane_id % 8) * 16
        ...
        def _load_lds(gl_src, lds_dst, k_offset, gl_offsets, n_steps):
            ...
            for step in range_constexpr(n_steps):
                ...
                lds_ptr = buffer_ops.create_llvm_ptr(
                    lds_base + fx.Int32(wave_id * 1024 + step * 8192), address_space=3
                )
        ...
        _load_lds(B_rsrc, B_lds[0][0], B0_gl_offset + 0 * BLOCK_K, global_offsets, N_LDS_STEPS_B)
        _load_lds(A_rsrc, A_lds[0][0], A0_gl_offset + 0 * BLOCK_K, global_offsets, N_LDS_STEPS_A)
        _load_lds(B_rsrc, B_lds[0][1], B128_gl_offset + 0 * BLOCK_K, global_offsets, N_LDS_STEPS_B)
        _load_lds(A_rsrc, A_lds[0][1], A128_gl_offset + 0 * BLOCK_K, global_offsets, N_LDS_STEPS_A)

        if wave_i == 1:
            rocdl.s_barrier()

        # Issue load for next tile
        _load_lds(B_rsrc, B_lds[1][0], B0_gl_offset + 1 * BLOCK_K, global_offsets, N_LDS_STEPS_B)
        _load_lds(A_rsrc, A_lds[1][0], A0_gl_offset + 1 * BLOCK_K, global_offsets, N_LDS_STEPS_A)
        _load_lds(B_rsrc, B_lds[1][1], B128_gl_offset + 1 * BLOCK_K, global_offsets, N_LDS_STEPS_B)
        ...
        for k in range_constexpr(K_ITERS - 2):
            ...
            b0_frag = _load_rt(B_lds[cur][0], wave_j, N_TILES_B)
            a0_frag = _load_rt(A_lds[cur][0], wave_i, N_TILES_A)
            _load_lds(A_rsrc, A_lds[next][1], A128_gl_offset + (k + 1) * BLOCK_K, global_offsets, N_LDS_STEPS_A)
            rocdl.s_barrier()
            ...
            _load_lds(B_rsrc, B_lds[cur][0], B0_gl_offset + (k + 2) * BLOCK_K, global_offsets, N_LDS_STEPS_B)
            rocdl.s_barrier()
            ...
            _load_lds(A_rsrc, A_lds[cur][0], A0_gl_offset + (k + 2) * BLOCK_K, global_offsets, N_LDS_STEPS_A)
            rocdl.s_barrier()
            ...
            _load_lds(B_rsrc, B_lds[cur][1], B128_gl_offset + (k + 2) * BLOCK_K, global_offsets, N_LDS_STEPS_B)
