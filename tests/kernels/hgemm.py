...
from tests.kernels.tensor_shim import (
    get_dtype_in_kernel,
    GTensor,
    STensor,
    _to_raw,
...
def compile_hgemm_ht_kernel(
    ...
    LDG_REG_A_COUNT_AS = HALF_BLOCK_M * BLOCK_K // BLOCK_THREADS // LDG_ASYNC_VEC_SIZE
    ...
    def hgemm_ht_kernel(
        ...
        for i in range_constexpr(LDG_REG_A_COUNT_AS):
            ...
            k_local_idx = global_tid % LDG_X_THREADS_AS * LDG_ASYNC_VEC_SIZE
...
def selection_filter(m, n, k, kwargs):
    ...
    if USE_HT:
        if not (STAGES == 2 and BLOCK_K_WARPS == 1 and B_TO_LDS == True):
            return False
